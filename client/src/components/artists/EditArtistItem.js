...
const EditArtistItem = ({
  ...
  const mailtoSubject = encodeURIComponent("Hey, " + theArtist.firstName + "! Just following up about our Zoom conversation on " + theArtist.typeformDate);
  const mailtoBody = encodeURIComponent(
`When we talked, you mentioned that you're from `+ theArtist.hometown + `.
What is it like there this time of year?`
);

  return (
      <div className={`profile bg-dark p-1 my-1 ${theArtist.active ? "active" : ""}`}>
        <Fragment>
        <img src={theArtist.squareImg} alt="" className="profileImage" />
        <div className="artistAdminButtons">
        <Link to={`/artists/${theArtist.slug}`} className='btn btn-primary profileBtn'>
            View Frontend Profile
        </Link>
            <a href={`mailto:${theArtist.email}?subject=${mailtoSubject}&body=${mailtoBody}`} className='btn btn-primary'><i className="fas fa-envelope"></i>
        ...
        {theArtist.active && (
          <small>Active</small>
            ...
            {theArtist.repLink ? (
                    <a href={theArtist.repLink} className='btn btn-primary' target="_blank" rel="noopener noreferrer">Check out {theArtist.stageName}'s work</a>
                ): ''}
                <button
